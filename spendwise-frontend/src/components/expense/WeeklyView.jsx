  ...
  const SummaryCards = () => (
    <div className="grid grid-cols-2 gap-3 mb-4">
      <div className={`${t.card} ${t.border} border rounded-2xl p-4 shadow-xs`}>
        <p className={`text-[11px] font-bold uppercase tracking-wider mb-1 ${t.muted}`}>This Week</p>
        <p className={`text-xl font-black tabular-nums ${t.text}`}>₹{total.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</p>
      </div>
      <div className={`${t.card} ${t.border} border rounded-2xl p-4 shadow-xs`}>
          ...
          return (
            <div key={cat} className="space-y-1">
              <div className="flex justify-between text-xs tabular-nums">
                ...
                <span className={`font-semibold ${t.muted}`}>₹{amt.toLocaleString("en-IN", { minimumFractionDigits: 2 })} · <strong className={t.text}>{pct.toFixed(0)}%</strong></span>
              </div>
