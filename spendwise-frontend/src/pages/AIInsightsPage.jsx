      ...
      <div className={`flex rounded-xl overflow-hidden border ${t.border} p-0.5 bg-black/5 dark:bg-white/5 max-w-md animate-fade-slide-up-1`}>
        {[
          ...
          <button key={key} onClick={() => setActiveTab(key)}
            className={`flex-1 py-2 rounded-lg text-xs font-extrabold transition-all duration-150 ${activeTab === key ? `${t.btn.primary} shadow-2xs` : "opacity-60 hover:opacity-100"}`}>
            {label}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4 animate-fade-slide-up-2">
          {!insights && !loading && (
            <div className={`${t.card} ${t.border} border rounded-2xl p-6 sm:p-8 text-center space-y-3.5 shadow-xs max-w-2xl mx-auto`}>
              <div className="w-14 h-14 mx-auto rounded-2xl bg-[#8B7CF6]/10 text-[#8B7CF6] flex items-center justify-center text-2xl">✨</div>
              <div>
              ...
              <button onClick={fetchInsights}
                className={`${t.btn.primary} w-full py-3.5 rounded-xl text-xs font-extrabold uppercase tracking-wider active:scale-98 transition-all shadow-sm`}>
                Analyze My Spending
              </button>
            </div>
          )}
          ...
          {insights && !loading && (
            <div className="space-y-4 animate-fade-slide-up">
                ...
                <div className={`${t.card} ${t.border} border rounded-2xl p-5 space-y-3 shadow-xs`}>
                  ...
                  <div className="space-y-2.5">
                    {insights.tips?.map((tip, i) => (
                      <div key={i} className="flex gap-3 items-start">
                        <span className="w-5 h-5 rounded-full bg-[#4F9D69]/15 text-[#4F9D69] font-black text-xs flex items-center justify-center shrink-0 mt-0.5">✓</span>
              ...
              <button onClick={fetchInsights}
                className={`${t.btn.secondary} w-full py-3 rounded-xl text-xs font-bold uppercase tracking-wider active:scale-98 transition-all shadow-2xs`}>
                Refresh Analysis
              </button>
            </div>
          )}
